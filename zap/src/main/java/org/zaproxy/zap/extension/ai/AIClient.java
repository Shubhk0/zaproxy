...
import org.zaproxy.zap.extension.ai.knowledge.WebSearchEngine;
...
public class AIClient {
    ...
    /**
     * Analyzes a ZAP Alert using the AI, optionally performing a web search for context.
     *
     * @param alert The alert to analyze.
     * @param enableWebSearch Whether to search the web for additional info.
     * @param onResponse Callback for the response.
     * @param onComplete Callback for completion.
     */
    public void analyzeAlert(Alert alert, boolean enableWebSearch, Consumer<String> onResponse, Consumer<String> onComplete) {
        executor.submit(() -> {
            String searchContext = "";
            if (enableWebSearch) {
                onResponse.accept("[AI: Searching web for '" + alert.getName() + " vulnerability'...]\\n");
                List<String> results = searchEngine.search(alert.getName() + " vulnerability exploit remediation");
                if (!results.isEmpty()) {
                    ...
                    onResponse.accept("[AI: Found " + results.size() + " web results. Analyzing...]\\n");
                } else {
                    onResponse.accept("[AI: No relevant web results found. Using internal knowledge only.]\\n");
                }
            }
    ...
    // Maintain backward compatibility or for simple calls
    public void analyzeAlert(Alert alert, Consumer<String> onResponse, Consumer<String> onComplete) {
        analyzeAlert(alert, false, onResponse, onComplete);
    }
